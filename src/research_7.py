...
def job(path):
    ...
    while ret:
        ...
        if ret and frame_idx % interval == 0:
            ...
            distance_array = pairwise_distances(des2, des1)
            closest_kp1_idx = np.argmin(distance_array, axis=1)

            for kp2_idx in range(distance_array.shape[0]):
                kp1_idx = closest_kp1_idx[kp2_idx]
