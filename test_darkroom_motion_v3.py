...
修正 v2 的問題:
1. ✅ 旋轉後再建立 ROI 遮罩（避免特徵點落在黑色區域）
2. ✅ 回退 ROI 至 0.6（更保守的範圍）
3. ✅ 確保視覺化輸出顯示旋轉後影像
...
ROI_RATIO = 0.6       # 回退到 0.6
MIN_MATCHES = 6       # 最低匹配對數

# 輸出目錄
OUTPUT_DIR = "lifts/darkroom_motion_test_v3"
...
def test_motion_detection():
    ...
    print("="*70)
    print("🔧 v3 修正項目:")
    print("  1. ✅ 旋轉後再建立 ROI 遮罩（避免黑邊干擾）")
    print("  2. ✅ ROI 回退到 0.6（更保守）")
    print("  3. ✅ 確保視覺化輸出正確旋轉")
    print("="*70)
    ...
    print(f"  ROI 範圍 (旋轉後): ({roi_x1}, {roi_y1}) - ({roi_x2}, {roi_y2})")
    print(f"  ROI 大小: {roi_x2-roi_x1}x{roi_y2-roi_y1}")
    ...
    # 標註資訊
    cv2.putText(vis_frame1, f"Rotated: {rotation_angle} deg",
               (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 0), 2)
    cv2.putText(vis_frame1, f"ROI: {ROI_RATIO}",
               (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 0), 2)
    cv2.putText(vis_frame1, f"Features: {len(kp1)}",
               (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)

    cv2.imwrite(os.path.join(OUTPUT_DIR, "frame1_features_v3.jpg"), vis_frame1)
    ...
    while True:
        ...
        if abs(time_sec - MOTION_POINT) < 3:  # 運動點附近 ±3 秒
            ...
            filename = f"frame_{frame_idx}_{time_sec:.1f}s_{status}_v3.jpg"
    ...
    # 與前版本比較
    print(f"\n📈 版本對比:")
    print(f"  v1 (無旋轉, ROI 0.6):     特徵點 22.0 | 匹配 12.9 | 成功率 98.0%")
    print(f"  v2 (旋轉但ROI錯誤):       特徵點 22.0 | 匹配 15.8 | 成功率 91.9%")
    print(f"  v3 (旋轉正確, ROI 0.6):  特徵點 {avg_kp:.1f} | 匹配 {avg_matches:.1f} | 成功率 {(motion_frames+static_frames)/total_frames*100:.1f}%")
    ...
    # 儲存詳細報告
    report_path = os.path.join(OUTPUT_DIR, "test_report_v3.txt")
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write("暗房運動偵測驗證報告 v3\n")
        f.write("="*70 + "\n\n")
        f.write("v3 修正:\n")
        f.write("  - 旋轉後再建立 ROI 遮罩\n")
        f.write("  - ROI 回退到 0.6\n")
        f.write("  - 確保視覺化輸出正確\n\n")
