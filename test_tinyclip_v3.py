"""
測試短片段 (21a_tinyclip.mp4) - 使用 v3 CLAHE 演算法
"""
...
ROI_RATIO = 0.6
FRAME_INTERVAL = 6
OUTPUT_DIR = r"D:\Lift_project\lifts\test_short\results_v3_clahe"

# Motion thresholds (adjusted for slow motion)
MOTION_MIN_MATCHES = 8
MOTION_MIN_DISPLACEMENT = 1.5  # Lowered for slow microscope movement
...
def draw_matches_and_motion(frame, kp, matches, vertical_displacements, roi_bounds, result_type):
    """繪製特徵點、匹配線、ROI 框"""
    vis = frame.copy()
...
if __name__ == "__main__":
    ...
    while True:
        ...
        frame_enhanced = preprocess_clahe(frame_rotated)

        # 4. Feature detection
        kp, desc = orb.detectAndCompute(frame_enhanced, mask)
        ...
        if prev_desc is not None and desc is not None:
            ...
            if len(good_matches) >= MOTION_MIN_MATCHES:
                ...
                vis = draw_matches_and_motion(frame_rotated, kp, match_pairs,
                                             vertical_displacements, roi_bounds, result_type)

                # Save image
                output_path = os.path.join(OUTPUT_DIR,
                    f"frame_{frame_idx:05d}_{timestamp:.1f}s_{result_type}_v3clahe.jpg")
