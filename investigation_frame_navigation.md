...
### 4.1) 新增實證觀察（使用者回報）
你提供了多組「主程式 inspection 疊字的 Frame ID」與「原始檔（PotPlayer/OpenCV）目視對位後的 Frame ID」近似對應：

| 主程式 Frame ID | 原檔（PotPlayer/OpenCV）近似對應 | 差值（原檔 − 主程式） |
|---:|---:|---:|
| 6858 | 7139 | +281 |
| 6906 | 7187 | +281 |
| 9000 | 9270 | +270 |
...
可見「差值」會隨著幀號變大而近似線性縮小，直到片尾趨近於 0。這不像固定常數偏移，也不像 FPS 漂移（會隨時間擴大），更像是一個「帶截距的線性對應」。以這些點做線性擬合，可得到與觀察高度一致的近似關係：

```
原檔幀（PotPlayer/OpenCV） ≈ 318 + 0.9946 × 主程式幀
```

驗證：
- 6906 → 318 + 0.9946×6906 ≈ 7186（對應 7187）
- 9000 → ≈ 9269（對應 9270）
- 20274 → ≈ 20481（對應 20481）
- 58836 → ≈ 58835～58836（對應 58836）

等價地，差值近似：

```
差值（原檔 − 主程式） ≈ 318 − 0.0054 × 主程式幀
```

解讀：
- 片頭相當於「主程式幀 0 ≈ 原檔幀 318」（起點偏移約 318 幀）。
- 之後主程式幀號成長略快於原檔（斜率 < 1），導致差值線性收斂，片尾幾乎對齊。

---

### 4.2) probe 實測摘要（只處理 1.mp4）
- `fps=59.675300`，但 `CAP_PROP_POS_MSEC` 的節拍呈現近似 16.667ms（60fps 格點）等距增長：
  - 例如：pre_idx=2000 → pre_ms=38650ms、pre_idx=4000 → 71983.333ms。
- `pre_idx` 與 `post_idx` 始終維持 `post_idx = pre_idx + 1`（read 前後一致）。
- 關鍵幀的時間戳：
  - 6858 → 119616.667ms、6906 → 120416.667ms、9000 → 155316.667ms、20274 → 343216.667ms、58836 → 985950.000ms。
- 結論：主程式這一路的時間軸回報呈現「接近 60fps 的時間格點」，與原檔實際 fps 59.675 的標度有微差，與 4.1 小節的線性關係相吻合。
...
#### 5.1) 臨時導航對齊方案（僅為輔助、可開關）
- 導航前對 CSV/inspection 的「主程式幀」應用仿射修正：

```
source_frame ≈ intercept + slope × main_frame
（預設 intercept=318, slope=0.9946，可經由環境變數覆寫）
```

- 建議在 `manual_correction_tool.py` 提供 helper，並以環境變數控制是否啟用：
  - `LIFT_MAP_FRAMES=1` 開啟（預設 0 關閉）
  - `LIFT_AFFINE_INTERCEPT`（預設 318）
  - `LIFT_AFFINE_SLOPE`（預設 0.9946）
- 僅在顯示與取幀時套用修正，不改動 CSV 原資料，利於回溯。
...
建議驗證一支片長明顯不同的影片，看看「到片尾是否同樣歸零」：

1) 先用我們的 probe 快跑一次，只處理目標檔並少量輸出：

設定只跑單一檔案與大間隔印點
PowerShell:
$env:LIFT_PROBE="1"; $env:LIFT_PROBE_INTERVAL="10000"; $env:LIFT_PROBE_FRAMES=""; $env:LIFT_TARGET="31-2.mp4"; uv run python -u src\lift_travel_detection.py


2) 取兩個人工對位點（片頭一個、片尾附近一個），用它們解一條線 F_src = a + b F_main：

b 先固定為 fps_source/60；由尾點 F_src_end≈F_main_end 解出 a ≈ (1−b)×F_end

或由兩點同時解 a、b，比較 b 是否接近 59.675/60


3) 把得到的 a、b 填到手動工具的參數跑看看：
uv run python src\manual_correction_tool.py --map-frames --map-intercept <a> --map-slope <b>

實務建議

先把 slope 固定為 fps_source/60（每檔由 OpenCV 讀到的 fps_source），intercept 用「片尾對齊」快速求解；若仍有小殘差，再用一個片頭對位點微調 intercept。
若這兩支片都符合「片尾對齊→intercept 隨片長線性變化」，即可把這一套規則當成預設。需要更精準時，再做每檔一次性的 intercept 校正。